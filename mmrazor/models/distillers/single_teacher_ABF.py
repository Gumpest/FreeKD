    ...
    def build_align_module(self, cfg):
        ...
        if cfg.get('num_modules', 1) == 1:
            if cfg.type == 'conv2d':
                ...
                align_module = ABF(in_channels, 2048, out_channels, fuse=True)
            ...
            for i in range(cfg.num_modules):
                if cfg.type == 'conv2d':
                    ...
                    align_module_ = ABF(in_channels[i], in_channels[i], out_channels[i], fuse=True)
                elif cfg.type == 'linear':
                ...
                if not isinstance(align_module, nn.ModuleList):
                    ...
                    if len(align_module) == len(student_outputs):
                        ...
                        for s_out in student_outputs:
                            ...
                            res = None
                            outs = []
                            # for m, x in zip(align_module, s_out):
                            for i in range(len(align_module)-1, -1, -1):
                                m, x = align_module[i], s_out[i]
                                if res is None:
                                    out, res = m(x)
                                else:
                                    out, res = m(x, res, (x.shape[-2], x.shape[-1]))
                                outs.insert(0, out)
                            s_out = tuple(outs)
